            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-bold text-lg text-base-content">{flag.name}</h3>
                  <span
                    className={`badge badge-sm font-semibold ${
                      flag.isEnabled ? 'badge-success text-white' : 'badge-ghost opacity-70'
                    }`}
                  >
                ...
                {/* Key Copy Pill */}
                <div className="inline-flex items-center gap-1 bg-base-200 px-2 py-0.5 rounded text-xs font-
  mono text-base-content/80 group">
                  <span>{flag.key}</span>
                  <button
                    onClick={handleCopy}
                    title="Copy flag key"
                    className="btn btn-ghost btn-xs p-0.5 h-auto min-h-0 text-base-content/60 hover:text-
  primary"
                  >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {/* Rollout % */}
              <div className="bg-base-200 p-2.5 rounded-lg flex flex-col gap-1">
                <div className="flex items-center justify-between text-base-content/70">
                  <span className="flex items-center gap-1">
                    <Percent className="w-3.5 h-3.5" /> Rollout
                  </span>
                  <span className="font-bold text-base-content">{flag.rolloutPercentage}%</span>
                </div>
                {/* Progress bar */}
                <progress
              ...
              {/* Target Whitelist */}
              <div className="bg-base-200 p-2.5 rounded-lg flex flex-col justify-between">
                <span className="flex items-center gap-1 text-base-content/70">
                  <Users className="w-3.5 h-3.5" /> Whitelist
                </span>
                <span className="font-semibold text-base-content mt-1">
